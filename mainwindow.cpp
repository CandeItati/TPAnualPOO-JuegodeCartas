...
{
    QPushButton *botonesJugador[5] = {ui->PBelemento1,          // Arreglo de punteros a botones. (No los crea solo guarda sus direcciones)
                                      ui->PBelemento2,
                                      ui->PBelemento3,
                                      ui->PBelemento4,
                                      ui->PBelemento5};
...
void MainWindow::mostrarAcciones()   //Hace exactamente lo mismo que antes pero lo contrario. Oculta los botones de elementos y muesta atacar y cambiar
{
...
void MainWindow::actualizarPantalla() //Actualiza toda la interfaz
{
    ...
    const vector<Elemento> &elementosJH = jugador1->getElementos(); //Obtiene el vector del jugador humano sin realizar una copia.
    const vector<Elemento> &elementosIA = jugador2->getElementos(); //Obtiene el vector del jugador IA sin realizar una copia.
    ...
    for (int i = 0; i < 5; i++) {       //Recorre los cinco elementos.
        ...
        botonesJugador[i]->setEnabled(elementosJH[i].estaVivo());       //Si el elemento murió deshabilita el boton

        vidasJugador[i]->setText(elementosJH[i].obtenerNombre() + ": "
                                 + QString::number(elementosJH[i].obtenerVida()) + "%");      //Muestra los elementos del Jugador humano con sus vidas

        vidasIA[i]->setText(elementosIA[i].obtenerNombre() + ": "
                            + QString::number(elementosIA[i].obtenerVida()) + "%");        //Muestra los elementos del Jugador IA con sus vidas
    }
